...
const ProfileInfo = () => {
    ...
    const updateUserDetailsFunc = () => {
        ...
        fetch(getApiBasicUrl + "/user-info-update", {
            method: "POST", // or 'PUT'
            headers: {
    ...
    const userProfilePictureUpdate = async (profileFile) => {
        ...
        try {
            const response = await fetch(`${getApiBasicUrl}/user-profile-picture-update`,
                {
                    method: "POST",
                    headers: {
                        'Authorization': 'bearer ' + getToken
        ...
        // fetch(getApiBasicUrl + "/user-profile-picture-update", {
        //     method: "POST", // or 'PUT'
        //     headers: {
        //         "Content-Type": "application/json",
        //         'Authorization': 'bearer ' + getToken
    ...
    const userPasswordUpdate = () => {
        ...
        if (previousPassword.length > 0 && password.length > 0 && confirmPassword.length > 0) {
            fetch(getApiBasicUrl + "/reset-password", {
                method: "POST", // or 'PUT'
                headers: {
                    "Content-Type": "application/json",
                    'Authorization': 'bearer ' + getToken
    ...
    useEffect(() => {
        ...
        fetch(getApiBasicUrl + '/user-info', {
            headers: {
                'Authorization': 'bearer ' + getToken,
                ...
                {isDiv3Visible &&
                    <div className="w-full h-full  rounded-lg flex flex-col md:flex-row justify-evenly items-center">
                        ...
                        >
                            ...
                            {getProfileImage && (
                                <img
                                    className="h-full w-full bg-gray-400 rounded-full"
                                    src={getProfileImage}
                                // alt="Updated Image"
                                />
                            )}
