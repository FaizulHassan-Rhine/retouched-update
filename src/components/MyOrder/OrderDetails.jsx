...
const OrderDetails = () => {
    ...
    const InvoiceFunc = (imageType) => {

        console.log(`${getApiBasicUrl}/cost-breakdown?order_master_image_id=${orderData.id}`)
        setImgType(imageType);

        fetch(`${getApiBasicUrl}/cost-breakdown?order_master_image_id=${orderData.id}`, {
            headers: {
            ...
            .then(data => {
                console.log(data);
                setInvoiceInfo(data);
                data.status_code == 200 && data.results.downloadable_image_count > 0 ? setModalOpen(true) : downloadFunc(imageType);
            })
    ...
    const downloadFunc = (imageType) => {
        console.log(getApiBasicUrl + `/get-order-image-detail-by-order-master-image-id?order_master_iamge_id=${orderData.id}`)
        fetch(getApiBasicUrl + `/get-order-image-detail-by-order-master-image-id?order_master_iamge_id=${orderData.id}`, {
            headers: {
                'Authorization': 'bearer ' + getToken,
                'Content-Type': 'application/x-www-form-urlencoded'
            }
            ...
            .then(data => {
                console.log(data);
                // console.log(data.results.order_image_detial)
    ...
    return (
        <>
            <div className='bg-[#FAFAFA] h-screen'>
                ...
                <div className='relative'>
                    ...
                    <div className='container mx-auto py-6 relative'>
                        ...
                        <h2 className="text-2xl mb-6 text-green-900 font-bold"><i className="fa-solid mr-2 fa-basket-shopping"></i>ORDER DETAILS  {id}</h2>
                        ...
                        <div className='grid grid-cols-4 justify-items-center gap-y-6'>

                            {
                                orderData.orderImageDetails.map((data, index) => {
                                    return (
                                        <div className='bg-white shadow-2xl rounded-xl w-64 h-48 flex justify-center items-center'>
            ...
            {
                isModalOpen && <FullScreenModal invoiceInfo={invoiceInfo} confirmCallback={confirmCallback} modalCallBack={modalCallBack} imageType={getImgType} />
            }
