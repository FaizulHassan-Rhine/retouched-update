...
const Modal = ({invoiceInfo, confirmCallback, modalCallBack, imageType }) => {
    ...
    const closeModal = () => {
     ...
     const DownloadFunc = () => {
        ...
        console.log(orderId);
        fetch('http://192.168.1.7:8001/api/2024-04/reduce-user-points-on-download', {
          method: 'POST',
          headers: {
...
{Object.keys(invoiceInfo).length > 0 && typeof invoiceInfo.results.order_master_charge_breakdown !== 'undefined' && invoiceInfo.results && invoiceInfo.results.order_master_charge_breakdown && invoiceInfo.results.order_master_charge_breakdown.length > 0 &&
                        <p className="font-semibold text-lg pt-5 ">Current Balance : <span className="font-normal"> {invoiceInfo.results.order_master_charge_breakdown[0].current_balance}</span> Credit(s)</p>
                    }
